...
def parse_last_name(display_name):
    """
    Infer last name from Canvas display name.
    Handles:
      - "Last, First Middle"
      - "First Middle Last"
    """
    ...
    if "," in display_name:
        last = display_name.split(",", 1)[0].strip()
...
def build_files_list(folder, allowed_ext=None):
    """
    Returns list of dicts with:
      - path
      - basename
      - stem_letters (letters-only stem, for prefix match)
    Matching rule: file stem letters start with student's last name letters.
    """
    ...
    for entry in os.listdir(folder):
        ...
        files.append({
            "path": path,
            "basename": entry,
            "stem_letters": norm_letters(stem),
        })
...
def prompt_for_assignment_id(course):
    ...
    # Validate it exists / you can access it
    try:
...
def count_attempts(submission_detail):
    ...
    if isinstance(hist, list) and hist:
        attempts = {int(h["attempt"]) for h in hist if h.get("attempt") is not None}
        return len(attempts) if attempts else len(hist)
...
def upload_comment_file_and_post_comment(api_url, api_key, course_id, assignment_id, user_id, file_path, comment_text):
    ...
    if not file_id:
        raise RuntimeError(f"Upload completed but could not determine file id for {file_name} (user_id={user_id}).")
...
def summarize_terminal(report):
    ...
    dup_names = []
    for d in report["canvas_duplicate_last_names"]:
    ...
    print("=== SUMMARY ===")
    print(f"Assignment ID used: {report.get('assignment_id_used')}")
    ...
    print(f"Skipped due to duplicate last names in Canvas: {len(dup_names)} {join_names(dup_names)}")
...
# Main logic
# ----------------------------
def auto_comment_from_files(api_url, api_key, course_id, assignment_id, files_folder, comment_text, allowed_ext=None, dry_run=False, report_path=None):
    canvas = Canvas(api_url, api_key)
    course = canvas.get_course(course_id)
    assignment = course.get_assignment(assignment_id)

    files = build_files_list(files_folder, allowed_ext=allowed_ext)
    ...
    for sub in subs:
        ...
        last = parse_last_name(sortable or name)
        candidates.append({
            "user_id": uid,
            "name": name,
            "sortable_name": sortable,
            "last_key": norm_letters(last),
        })

    canvas_by_last = defaultdict(list)
    for row in candidates:
        canvas_by_last[row["last_key"]].append(row)

    report = {
        "assignment_id_used": assignment_id,
        "canvas_missing_file": [],
        "canvas_duplicate_last_names": [],
    ...
    # duplicates (skip)
    for last_key, rows in canvas_by_last.items():
        if last_key and len(rows) > 1:
            report["canvas_duplicate_last_names"].append({
                "last_key": last_key,
                "students": [{"user_id": r["user_id"], "name": r["name"], "sortable_name": r["sortable_name"]} for r in rows]
            })

    # file has no associated student (heuristic): no student's last_key is prefix of file stem
    last_keys = set(k for k in canvas_by_last.keys() if k)
    for f in files:
        if not any(f["stem_letters"].startswith(k) for k in last_keys):
            report["file_missing_canvas_student"].append({"files": [f["path"]]})

    # process unique last names only
    for last_key, rows in canvas_by_last.items():
        if not last_key:
            for r in rows:
                report["canvas_missing_file"].append({
                    ...
                    "reason": "Could not parse last name from Canvas user name."
                })
            continue
        ...
        r = rows[0]
        matching_files = [f["path"] for f in files if f["stem_letters"].startswith(last_key)]
        ...
        if len(matching_files) == 0:
            report["canvas_missing_file"].append({
                ...
                "sortable_name": r["sortable_name"],
                "last_key": last_key,
                "reason": "No matching file found (by last-name prefix)."
            })
            continue
        ...
        # detail check (multiple attempts + existing attachment)
        try:
        ...
        try:
            file_id = upload_comment_file_and_post_comment(
                api_url, api_key, course_id, assignment_id, r["user_id"], file_path, comment_text
            )
            report["uploaded"].append({
                "user_id": r["user_id"],
                "name": r["name"],
